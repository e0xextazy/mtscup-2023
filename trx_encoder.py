...
class TrxEncoder(TrxEncoderBase):
    ...
    Parameters
        ...
        text_value:
            dict with text feature name.
            Values must be like this `{'in': dictionary_size, 'out': embedding_size, 'name': text_col_name}`
            If you have pretrained weights must be like this `{'weight': weight, 'name': text_col_name}`.
            Where weight is 2d numpy array
    ...
    def __init__(self,
        ...
        if text_value is not None:
            ...
            if 'weight' in text_value:
                self.text_embs = nn.Embedding.from_pretrained(torch.Tensor(text_value['weight']))
            else:
                self.text_embs = nn.Embedding(text_value['in'], text_value['out'], padding_idx=0)
        else:
    ...
    def forward(self, x: PaddedBatch):
        ...
        if self.text_value is not None:
            text_embs = self.text_embs(x.payload[self.text_value["name"]])
            text_embs = text_embs.mean(dim=2)
            processed_embeddings.append(text_embs)
